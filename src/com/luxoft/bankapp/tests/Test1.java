...
public class Test1 {
	...
	public void testBank() throws ClientExistsException {
		...
		Iterator<Client> iteratorClients = bank.getClients().iterator();

		assertEquals(2, bank.getClients().size());

		Client firstClient = iteratorClients.next();
		Client secondClient = iteratorClients.next();
	...
	@Test
	public void testSameAccountName() throws ClientExistsException {
		Bank bank = new Bank();
		Client client1 = new Client("Smith John", Gender.MALE);
		client1.addAccount(new SavingAccount(1, 1000.0));
		client1.addAccount(new CheckingAccount(2, 1000.0, 100.0));
		...
		Client client3 = new Client("Smith John", Gender.MALE);
		client3.addAccount(new SavingAccount(1, 1000.0));
		client3.addAccount(new CheckingAccount(2, 1000.0, 100.0));

		BankService.addClient(bank, client1);

		assertThrows(ClientExistsException.class, () -> BankService.addClient(bank, client3));
	}
